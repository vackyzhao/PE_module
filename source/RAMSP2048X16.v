...
//       
//       Creation Date:  Wed Nov 16 19:12:59 2022
//       Version: 	r1p1
//
...
`ifdef POWER_PINS
module RAMSP2048X16 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`else
module RAMSP2048X16 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`endif
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
      ...
      if (GWEN_int !== 1'b1) begin
        ...
        readLatch0 = {data_out[120], data_out[112], data_out[104], data_out[96], data_out[88],
...
`endif
  ...
  reg NOT_WEN0, NOT_A10, NOT_A9, NOT_A8, NOT_A7, NOT_A6, NOT_A5, NOT_A4, NOT_A3, NOT_A2;
  reg NOT_A1, NOT_A0, NOT_D15, NOT_D14, NOT_D13, NOT_D12, NOT_D11, NOT_D10, NOT_D9;
  reg NOT_D8, NOT_D7, NOT_D6, NOT_D5, NOT_D4, NOT_D3, NOT_D2, NOT_D1, NOT_D0, NOT_EMA2;
  reg NOT_EMA1, NOT_EMA0, NOT_EMAW1, NOT_EMAW0, NOT_GWEN, NOT_RET1N;
 ...
 wire  CLK_;
  ...
  buf B59(D_[14], D[14]);
  buf B60(D_[15], D[15]);
  buf B61(EMA_[0], EMA[0]);
  buf B62(EMA_[1], EMA[1]);
  buf B63(EMA_[2], EMA[2]);
  buf B64(EMAW_[0], EMAW[0]);
  buf B65(EMAW_[1], EMAW[1]);
  buf B66(GWEN_, GWEN);
  buf B67(RET1N_, RET1N);

   `ifdef ARM_FAULT_MODELING
     RAMSP2048X16_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module RAMSP2048X16_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [15:0] Q_out;
   input [15:0] Q_in;
   input CLK;
   input [10:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(11'd126,4'd6,2'd1,2'd0);
   `endif
end
   task add_fault;
